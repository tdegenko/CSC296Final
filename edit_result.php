...
$pkmn=Pokemon::findByAttrs(array("ID"=>$_GET["ID"],"originalTrainer"=>$_GET["originalTrainer"]));
?>
...
function changePoke($pok){
	...
	$mv1=moves::findByAttrs(array("name"=>$_GET["moveName1"]));
	if($_GET["moveName2"]=="")
    $mv2=moves::findByAttrs(array("name"=>$pok->getmoveName2()));
	else
	$mv2=moves::findByAttrs(array("name"=>$_GET["moveName2"]));
	if($_GET["moveName3"]=="")
    $mv3=moves::findByAttrs(array("name"=>$pok->getmoveName3()));
	else
	$mv3=moves::findByAttrs(array("name"=>$_GET["moveName3"]));
	if($_GET["moveName4"]=="")
    $mv4=moves::findByAttrs(array("name"=>$pok->getmoveName4()));
	else
	$mv4=moves::findByAttrs(array("name"=>$_GET["moveName4"]));
    if(count($itm)==1){
        $itm=$itm[0];
    }else{
        $itm=null;
